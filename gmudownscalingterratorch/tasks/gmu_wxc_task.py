...
class GmuWxcTask(BaseTask):
    ...
    def configure_models(self):
        ...
        _in_channels = self.hparams.get("in_channels", 160)
        _input_size_time = self.hparams.get("input_size_time", 2)
        _in_channels_static = self.hparams.get("in_channels_static", 8)
        _input_scalers_mu = self.in_mu
        _input_scalers_sigma = self.in_sig
        _input_scalers_epsilon = self.hparams.get("input_scalers_epsilon", 0.0)
        _static_input_scalers_mu = self.static_mu
        _static_input_scalers_sigma = self.static_sig
        _static_input_scalers_epsilon = self.hparams.get("static_input_scalers_epsilon", 0.0)
        _output_scalers = self.output_sig
        _n_lats_px = self.hparams.get("n_lats_px", 360)
        _n_lons_px = self.hparams.get("n_lons_px", 576)
        _default_patch_size_px = (2, 2)
        _patch_size_px = self.hparams.get("patch_size_px", _default_patch_size_px)
        _default_mask_unit_size_px = (30, 32)
        _mask_unit_size_px = self.hparams.get("mask_unit_size_px", _default_mask_unit_size_px)
        _mask_ratio_inputs = self.hparams.get("mask_ratio_inputs", 0.0)
        _mask_ratio_targets = self.hparams.get("mask_ratio_targets", 0.0)
        _embed_dim = self.hparams.get("embed_dim", 2560)
        _n_blocks_encoder = self.hparams.get("n_blocks_encoder", 12)
        _n_blocks_decoder = self.hparams.get("n_blocks_decoder", 2)
        _mlp_multiplier = self.hparams.get("mlp_multiplier", 4)
        _n_heads = self.hparams.get("n_heads", 16)
        _dropout = self.hparams.get("dropout", 0.0)
        _drop_path = self.hparams.get("drop_path", 0.0)
        _parameter_dropout = self.hparams.get("parameter_dropout", 0.0)
        _residual = self.hparams.get("residual", "climate")
        _masking_mode = self.hparams.get("masking_mode", "global")
        _positional_encoding = self.hparams.get("positional_encoding", "fourier")
        _encoder_shifting = self.hparams.get("encoder_shifting", False)
        _decoder_shifting = self.hparams.get("decoder_shifting", False)
        _checkpoint_encoder = self.hparams.get("checkpoint_encoder", [])
        _checkpoint_decoder = self.hparams.get("checkpoint_decoder", [])

        model = PrithviWxC(
            in_channels=_in_channels,
            input_size_time=_input_size_time,
            in_channels_static=_in_channels_static,
            input_scalers_mu=_input_scalers_mu,
            input_scalers_sigma=_input_scalers_sigma,
            input_scalers_epsilon=_input_scalers_epsilon,
            static_input_scalers_mu=_static_input_scalers_mu,
            static_input_scalers_sigma=_static_input_scalers_sigma,
            static_input_scalers_epsilon=_static_input_scalers_epsilon,
            output_scalers=_output_scalers,
            n_lats_px=_n_lats_px,
            n_lons_px=_n_lons_px,
            patch_size_px=_patch_size_px,
            mask_unit_size_px=_mask_unit_size_px,
            mask_ratio_inputs=_mask_ratio_inputs,
            mask_ratio_targets=_mask_ratio_targets,
            embed_dim=_embed_dim,
            n_blocks_encoder=_n_blocks_encoder,
            n_blocks_decoder=_n_blocks_decoder,
            mlp_multiplier=_mlp_multiplier,
            n_heads=_n_heads,
            dropout=_dropout,
            drop_path=_drop_path,
            parameter_dropout=_parameter_dropout,
            residual=_residual,
            masking_mode=_masking_mode,
            encoder_shifting=_encoder_shifting,
            decoder_shifting=_decoder_shifting,
            positional_encoding=_positional_encoding,
            checkpoint_encoder=_checkpoint_encoder,
            checkpoint_decoder=_checkpoint_decoder,
        )
        self._download_and_load_weights_for_prithvi_wxc()

        if (hasattr(self.model, "device") and self.model.device != _device) or not hasattr(
            self.model, "device"
        ):
