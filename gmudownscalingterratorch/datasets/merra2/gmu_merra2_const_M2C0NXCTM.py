...
class GmuMerra2M2C0NXCTM(RasterDataset):
    """
        const_2d_ctm_Nx (M2C0NXCTM)
        original data: MERRA2_400.const_2d_ctm_Nx.nc4
    """

    filename_glob = "MERRA2_[0-9][0-9][0-9].const_2d_ctm_Nx.*.tif"
    #Leave out 'date' group to allow RasterDataset to set it to maximum temporal range
    filename_regex = (
        r"MERRA2_[0-9][0-9][0-9]\.const_2d_ctm_Nx\."
    ...
    def __init__(
        ...
        if download:
            ...
            status = download_data_merra2_with_template(
                url_temp=self.M2C0NXCTM_BASE_URL_TEMPLATE,
        ...
        if extract:
            temp = self.M2C0NXCTM_BASE_URL_TEMPLATE
