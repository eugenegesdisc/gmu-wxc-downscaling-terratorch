...
class GmuMerra2M2T1NXLND(RasterDataset):
    """
        M2T1NXLND (tavg1_2d_lnd_Nx)
        original data: MERRA2_400.tavg1_2d_lnd_Nx.20200101.nc4
    """
    filename_glob = "MERRA2_[0-9]00.tavg1_2d_lnd_Nx.*.tif"
    filename_regex = (
        r"MERRA2_[0-9]00\.tavg1_2d_lnd_Nx\.(?P<olddate>\d{8})"
    ...
    is_image = True
    separate_files = True
    all_bands = ("GWETROOT", "LAI")

    M2T1NXLND_BASE_URL_TEMPLATE=(
        "https://goldsmr4.gesdisc.eosdis.nasa.gov/data/MERRA2/M2T1NXLND.5.12.4/"
        "{year}/{month}/"
        "MERRA2_{YYYY}00.tavg1_2d_lnd_Nx.{year}{month}{day}.nc4")
    ...
    def __init__(
        ...
        if download:
            ...
            status = download_data_merra2_with_template(
                url_temp=self.M2T1NXLND_BASE_URL_TEMPLATE,
        ...
        if extract:
            temp = self.M2T1NXLND_BASE_URL_TEMPLATE
