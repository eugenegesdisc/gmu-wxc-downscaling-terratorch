"""Module for GmuPrismDaily4km dataset"""
...
class GmuPrismDaily4km(RasterDataset):
    """
        PRISM Climate Date at 4km or 25m resolution
        original data: prism_ppt_us_25m_20200101.zip
        Datasets
            ppt - precipitation
    ...
    """
    filename_glob = "prism_*_25m_*.tif"
    ...
    PRISM4KMPPT_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "ppt/daily/{year}/prism_ppt_us_25m_{year}{month}{day}.zip")
    PRISM4KMTDMEAN_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "tdmean/daily/{year}/prism_tdmean_us_25m_{year}{month}{day}.zip")
    PRISM4KMTMAX_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "tmax/daily/{year}/prism_tmax_us_25m_{year}{month}{day}.zip")
    PRISM4KMTMEAN_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "tmean/daily/{year}/prism_tmean_us_25m_{year}{month}{day}.zip")
    PRISM4KMTMIN_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "tmin/daily/{year}/prism_tmin_us_25m_{year}{month}{day}.zip")
    PRISM4KMVPDMAX_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "vpdmax/daily/{year}/prism_vpdmax_us_25m_{year}{month}{day}.zip")
    PRISM4KMVPDMIN_BASE_URL_TEMPLATE=(
        "https://data.prism.oregonstate.edu/time_series/us/an/4km/"
        "vpdmin/daily/{year}/prism_vpdmin_us_25m_{year}{month}{day}.zip")
    def __init__(
            self,
        ...
        """
            Initialize a new GmuPrismDaily4km instance.
        ...
        """
        if not self._validate_bands(bands=bands, all_bands=self.all_bands):
            raise PrismUnknownElement(
                elem=bands.__str__(),
                msg="Not all elements are supported in PRISM climate data repo")
        if bands:
            self.all_bands = list()
            for band in bands:
        ...
        super().__init__(
            ...
            cache=cache)
    def _validate_bands(self,bands:list[str], all_bands:list[str])->bool:
        if bands is None:
            return True
        for bnd in bands:
    ...
    def _get_template(
        ...
        """
        if temp_attr == "ppt":
            return self.PRISM4KMPPT_BASE_URL_TEMPLATE
        elif temp_attr == "tdmean":
            return self.PRISM4KMTDMEAN_BASE_URL_TEMPLATE
        elif temp_attr == "tmax":
            return self.PRISM4KMTMAX_BASE_URL_TEMPLATE
        elif temp_attr == "tmin":
            return self.PRISM4KMTMIN_BASE_URL_TEMPLATE
        elif temp_attr == "tmean":
            return self.PRISM4KMTMEAN_BASE_URL_TEMPLATE
        elif temp_attr == "vpdmax":
            return self.PRISM4KMVPDMAX_BASE_URL_TEMPLATE
        elif temp_attr == "vpdmin":
            return self.PRISM4KMVPDMIN_BASE_URL_TEMPLATE
        else:
